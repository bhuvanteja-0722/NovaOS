...
- M11 fixed built-in user-probe image copied into the identity-mapped user page, validated user entry/stack context, process-specific ownership, guarded termination, protected probe mapping, validated ring-3 iret frame, and fail-closed transition checks; ring-3 entry remains disabled until actual mapped user execution and a tested return path exist.
