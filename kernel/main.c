...
extern int32_t syscall_interrupt_handler(const void *frame, uint32_t syscall_number, const void *saved);
...
void serial_write_bytes(const char *text, uint32_t length) {
...
void kmain(uint32_t multiboot_magic, uint32_t multiboot_info) {
    ...
    if (syscall_interrupt_handler(&synthetic_user_frame, NOVA_SYSCALL_GETPID, (const void *)0) != (int32_t)init_pid ||
        syscall_interrupt_handler(&synthetic_user_frame, NOVA_SYSCALL_YIELD, (const void *)0) != 0 ||
        syscall_interrupt_handler(&synthetic_user_frame, NOVA_SYSCALL_WRITE, (const void *)0) != -NOVA_EINVAL ||
        syscall_interrupt_handler(&synthetic_user_frame, 0xFFFFFFFFu, (const void *)0) != -NOVA_ENOSYS) {
        serial_write("ERROR: getpid/yield syscall dispatch validation failed\n");
        for (;;) {
            __asm__ volatile ("cli; hlt");
        }
    }
    syscall_interrupt_handler(&synthetic_user_frame, NOVA_SYSCALL_EXIT, (const void *)0);
